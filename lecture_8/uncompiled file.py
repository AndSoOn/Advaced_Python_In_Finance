...
class OU(object):
    ...
    def cond_u(self, x0, t):
        return self.u * (1.0 - math.exp(-self.k * t)) + x0 * math.exp(-self.k * t)

    def cond_std(self, x0, t):
        return np.sqrt(self.sig * self.sig / 2.0 / self.k * (1.0 - math.exp(-2 * self.k * t)))
    ...
    @staticmethod
    def decorr(kx, ky, dt):
        return 2 * math.sqrt(kx * ky) * (1.0 - math.exp(-dt * (kx + ky))) / (kx + ky) / math.sqrt((1.0 - math.exp(-2 * kx * dt)) * (1.0 - math.exp(-2 * ky * dt)))

    def __str__(self):
        return 'kappa = %.4f, mu = %.4f, sigma = %.4f, mean rev time = %.2f' % (self.k,
         self.u,
         self.sig,
         1.0 / self.k)
...
class GBM(object):
    ...
    def draw(self, es, x0, dt):
        ...
        for e in es.T:
            x = x + (self.u - 0.5 * self.vol * self.vol) * dt + self.vol * e * np.sqrt(dt)
            xs.append(x)
...
class CIR(object):
    ...
    def draw_log(self, es, x0, dt):
        ...
        for e in es.T:
            x = np.exp(y)
            y = y + (self.k * (self.u / x - 1.0) - 0.5 / x * self.vol * self.vol) * dt + self.vol * e * np.sqrt(dt / x)
